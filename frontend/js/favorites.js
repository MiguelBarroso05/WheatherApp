...
async function displayFavoritesWeatherData() {
    ...
    let backgroundImage = false; // To ensure the background is only set once

    favorites.forEach(async (city, id, index) => {
        ...
        try {
            ...
            // Set the background based on the weather condition, only once
            if (!backgroundImage) {
            ...
            if (error.message.startsWith('Error HTTP:'))
                errorMessage = `Error on API: ${error.message}`; // Display HTTP-specific errors
